...
<cfcomponent 
	...
	<cffunction name="filterEvent" access="public" returntype="boolean" output="false">
		...
		<cfif NOT (invalidEvent EQ '')>
			<cfset isContinue = authenticate(arguments.event.getArg("sessionListener")) />
		<cfelse>
	...
	<cffunction name="authenticate" access="public" returntype="boolean">
		<cfargument name="sessionFacade" type="calendar.models.sessionFacade" required="true" />
		<cfreturn arguments.sessionFacade.hasAccount() />		
	</cffunction>
	
	<!--- PROTECTED FUNCTIONS --->
	<cffunction name="throwUsageException" access="private" returntype="void" output="false">
		<cfset var throwMsg = "AuthenticationFilter requires the following usage parameters: " & this.INVALID_EVENT_PARAM & "." />
